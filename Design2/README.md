# Gymind Brand 2.0 Design Reference

## Overview
This directory contains the original interactive reference for Gymind's Brand 2.0 visual system. The production Expo application now implements the main design language across authentication, onboarding, home, workout logging, nutrition, progress, and navigation. Some prototype interactions remain future product work and are identified below.

## About the Design Files
`Gymind UI.dc.html` is a **design reference built in HTML** (a single-file interactive prototype), not production code. It is not React Native and should not be copied verbatim. Use it to understand the intended visual system; use the current `mobile/` implementation and root project documentation as the source of truth for runtime behavior.
...
### 6. Coach
The prototype includes a historical AI chat concept. It is not an active implementation specification. The previous Coach implementation was intentionally removed from the production application; the Coach tab now contains only a clean placeholder and no canned replies, local chat history, backend endpoint, or AI provider integration. Any rebuilt Coach should be designed and implemented incrementally against the current architecture.
...
## State Management
The HTML keeps all prototype state in one component for demonstration. The production application instead uses Expo Router screens, shared theme/session infrastructure, typed API calls, and screen-local React state. Prototype mock data is not a production data source. Historical Coach chat state is intentionally excluded from the current application.
