...
def test_cannot_favorite_another_users_private_exercise(client):
    owner_token = register_login_and_create_profile(
        client, "exerciseowner@example.com", "exerciseowner"
    )
    other_token = register_login_and_create_profile(
        client, "exerciseother@example.com", "exerciseother"
    )
    owner_headers = {"Authorization": f"Bearer {owner_token}"}
    other_headers = {"Authorization": f"Bearer {other_token}"}
    exercise_id = client.post(
        "/exercises",
        json={"name": "Owner's movement", "muscle_group": "core"},
        headers=owner_headers,
    ).json()["id"]

    response = client.post(f"/exercises/{exercise_id}/favorite", headers=other_headers)
