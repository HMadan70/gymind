...
def test_private_exercise_cannot_be_used_by_another_user(client):
    owner_token = register_login_and_create_profile(
        client, "privateexerciseowner@example.com", "privateexerciseowner"
    )
    other_token = register_login_and_create_profile(
        client, "privateexerciseother@example.com", "privateexerciseother"
    )
    ...
    exercise_id = client.post(
        "/exercises",
        json={"name": "Owner Only Lift", "muscle_group": "back"},
        headers=owner_headers,
    ).json()["id"]
    workout_id = client.post("/workouts", headers=other_headers).json()["id"]

    add_set = client.post(
        f"/workouts/{workout_id}/sets",
        json={"exercise_id": exercise_id, "set_number": 1, "weight": 100, "reps": 5},
    ...
    history = client.get(f"/exercises/{exercise_id}/history", headers=other_headers)
    progress = client.get(f"/progress/e1rm?exercise_id={exercise_id}", headers=other_headers)

    assert add_set.status_code == 404
    assert add_note.status_code == 404
...
def _make_workout_with_set(client, headers, weight=135, reps=8):
    """
    Build the fixture the edit tests all need: an exercise, a workout,
    and one logged set in it. Returns (workout_id, set_id).
    """
    exercise_id = client.post(
        "/exercises",
        json={"name": "Bench Press", "muscle_group": "chest"},
    ...
    set_id = client.post(
        f"/workouts/{workout_id}/sets",
        json={"exercise_id": exercise_id, "set_number": 1, "weight": weight, "reps": reps},
        headers=headers,
    ).json()["id"]
...
def test_can_edit_set_on_recently_finished_workout(client, db_session):
    token = register_login_and_create_profile(
        client, "editrecent@example.com", "editrecentuser"
    )
    headers = {"Authorization": f"Bearer {token}"}
    workout_id, set_id = _make_workout_with_set(client, headers)

    # finish it 2 days ago - inside the 7-day edit window
    workout = db_session.get(models.UserWorkout, workout_id)
    workout.ended_at = datetime.now(timezone.utc) - timedelta(days=2)
    db_session.commit()

    response = client.put(
        f"/workouts/{workout_id}/sets/{set_id}",
        json={"weight": 145, "reps": 6},
        headers=headers,
    )

    assert response.status_code == 200
    body = response.json()
    assert body["weight"] == 145
    assert body["reps"] == 6
    assert body["id"] == set_id


def test_cannot_edit_set_on_workout_finished_over_seven_days_ago(client, db_session):
    token = register_login_and_create_profile(
        client, "editold@example.com", "editolduser"
    )
    headers = {"Authorization": f"Bearer {token}"}
    workout_id, set_id = _make_workout_with_set(client, headers)

    # finished 8 days ago - outside the window
    workout = db_session.get(models.UserWorkout, workout_id)
    workout.ended_at = datetime.now(timezone.utc) - timedelta(days=8)
    db_session.commit()

    response = client.put(
        f"/workouts/{workout_id}/sets/{set_id}",
        json={"weight": 999, "reps": 1},
        headers=headers,
    )

    assert response.status_code == 409
    assert "can no longer be edited" in response.json()["detail"]

    # and the set really wasn't touched
    db_session.expire_all()
    unchanged = db_session.get(models.WorkoutSet, set_id)
    assert unchanged.weight == 135
    assert unchanged.reps == 8


def test_can_edit_set_on_unfinished_workout(client):
    token = register_login_and_create_profile(
        client, "editactive@example.com", "editactiveuser"
    )
    headers = {"Authorization": f"Bearer {token}"}
    workout_id, set_id = _make_workout_with_set(client, headers)

    # never finished - the window only starts once ended_at is set
    response = client.put(
        f"/workouts/{workout_id}/sets/{set_id}",
        json={"weight": 155, "reps": 5},
        headers=headers,
    )

    assert response.status_code == 200
    assert response.json()["weight"] == 155


def test_cannot_edit_another_users_set(client):
    token_a = register_login_and_create_profile(
        client, "editowner@example.com", "editowneruser"
    )
    token_b = register_login_and_create_profile(
        client, "editthief@example.com", "editthiefuser"
    )
    headers_a = {"Authorization": f"Bearer {token_a}"}
    workout_id, set_id = _make_workout_with_set(client, headers_a)
    ...
    # same not-found-if-not-yours convention as the rest of the app
    assert response.status_code == 404
    assert response.json()["detail"] == "Workout not found"


def test_editing_a_set_that_belongs_to_another_workout_is_404(client):
    token = register_login_and_create_profile(
        client, "editmismatch@example.com", "editmismatchuser"
    )
    headers = {"Authorization": f"Bearer {token}"}
    workout_id_one, set_id_one = _make_workout_with_set(client, headers)
    workout_id_two, _ = _make_workout_with_set(client, headers)

    # set belongs to workout one, but is addressed under workout two
    response = client.put(
        f"/workouts/{workout_id_two}/sets/{set_id_one}",
        json={"weight": 200, "reps": 3},
        headers=headers,
    )
