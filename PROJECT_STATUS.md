...
├── Design/
│   ├── exports/           # "5c" era screen images — superseded by Brand 2.0 (Section 14), still present
│   │                      # in the repo untouched/unarchived (19 files: 9 mobile, 9 web, 2 reference PNGs)
│   └── Source/            # "5c" era Claude Design source files (.dc, support.js) — superseded, still present
├── Design2/               # Brand 2.0 handoff (added to git 2026-09-05, commit e011f66) — BRAND_GUIDE.md,
│   └── brand/             # README.md, PUBLISHING_CHECKLIST.md, "Gymind UI.dc.html", brand/logo-mark.png,
...
| GET | `/workouts` | ✅ Built | Lists workout sessions for the current user, newest first. **As of 2026-09-05, returns `UserWorkoutListItemOut`** (adds `total_volume`/`total_sets` per workout via one grouped aggregate query, for the Past Workouts list's totals) **and fixes a bug where `started_at` was silently never returned** (see Section 10). Uses `require_profile`. |
...
| PUT | `/workouts/{id}/sets/{set_id}` | ✅ Built (2026-09-05) | Edits an already-logged set's `weight`/`reps` (both optional, partial update). Unlike `POST .../sets`, **not** blocked by a finished workout's `ended_at` — this route exists specifically to correct historical logs after the fact. Uses `require_profile`; checks `user_id` ownership on the parent workout, `404` if the set doesn't belong to it. |
...
| POST | `/nutrition/{id}/photo` | ✅ Built (2026-09-05) | Uploads a meal photo (multipart `file`, `image/jpeg`\|`png`\|`webp` only, `400` otherwise), saves it via `app/storage.py` (local volume — see Section 10), sets `nutrition_logs.photo_url`. Ownership-checked (`404` if not found or not theirs). Uses `require_profile`. |
...
| Method | Path | Status | Description |
|---|---|---|---|
| POST | `/progress-photos` | ✅ Built (2026-09-05) | Uploads a progress photo (multipart `file`, same content-type validation as the nutrition photo route), creates a `progress_photos` row. Uses `require_profile`. |
| GET | `/progress-photos` | ✅ Built | Lists the current user's own photos, newest first. Uses `require_profile`. |
| DELETE | `/progress-photos/{id}` | ✅ Built | Deletes a photo (DB row + the underlying file via `storage.delete_upload`, best-effort). Ownership-checked (`404` if not theirs). Uses `require_profile`. |
...
- **Alembic autogenerate caught a real model/DB mismatch — fixed 2026-09-05:** the first autogenerated migration revealed that `workout_sets.workout_id`'s `ForeignKey` declaration in `models.py` was missing `ondelete="CASCADE"`, even though the real database has that cascade behavior (set up manually via psql, verified earlier via `pg_constraint`). Fixed by adding `ondelete="CASCADE"` to the `ForeignKey(...)` call. **No migration was needed** — the DB already had the cascade, so this was a pure Python-side declaration fix; `alembic revision --autogenerate` confirmed no diff on `workout_sets` after the change (only the same pre-existing unrelated index/constraint drift every autogenerate run has shown since baselining, still not applied — see below). 36/36 tests still passing.
...
- **Frontend theme system (`mobile/src/constants/theme.ts` + `mobile/src/context/ThemeContext.tsx`):** a React Context (`ThemeProvider`/`useTheme`) providing `baseTokens` (dark/light), fixed `statusColors`, and per-preset accent tokens via `getAccentTokens(preset, mode)`. Accent presets match the real design system source exactly — 5 presets (Ember, Signal, Amber, Violet, Volt), no "Teal" (see Section 14). Consumed by screens as `const { colors } = useTheme()`.
...
- **Icon set: Lucide, not Phosphor (2026-09-05).** Both fit the brand's "simple/geometric, no filled/glossy/3D" requirement, but Lucide is single-weight by default (matches "regular weight only" with no risk of someone reaching for a bold/fill variant later) and is stroke-based with rounded caps/joins — closer to the design prototype's own hand-drawn icons (e.g. the logo mark's pulse-line polyline, `stroke-width:2.4;stroke-linecap:round`) than Phosphor's multi-weight (thin/light/regular/bold/fill/duotone) family. It's also built on `react-native-svg`, already a project dependency. Installed as `lucide-react-native`; every icon across the rebuilt screens comes from it.
- **App icon/splash wired (2026-09-05), and a real discrepancy found in `BRAND_GUIDE.md`/`README.md` while doing it:** both docs describe `brand/app-icon-1024.png` as "no transparency (App Store/Play Store spec)," but the actual file has real alpha variation (verified with Pillow: alpha channel ranges 0-255, not uniformly 255 — only the four rounded corners are transparent, everything else is opaque). iOS icons must be fully opaque or they render wrong, so `mobile/assets/images/icon.png` (used for both the top-level `icon` and `ios.icon`) is that file alpha-composited onto the brand dark base (`#040f15`) and flattened to RGB, not the raw export. The raw RGBA file (`android-icon-foreground.png`) is used as-is for Android's adaptive icon, where transparency is actually wanted — but the shape fills ~97% of its 1024 canvas edge-to-edge (not a real ~66% "safe zone"), so it's scaled to 70% and centered on a transparent canvas first, or launcher masking (circle/squircle) would clip the pulse-line tips. **Android's monochrome/themed icon (13+) has no source asset in the design** — there's no separate vector for just the pulse-line glyph, only the raster export with it baked over a gradient fill. Derived one anyway: thresholded the inset foreground for near-white pixels (the line is pure white, confirmed via pixel sampling; the gradient fill never is), keeping only those as an opaque white silhouette on transparent — this is an assumption/technique, not a design-spec value, flagged here in case the real design source ever provides a proper vector glyph to redo it from. Splash: `expo-splash-screen` plugin config (`backgroundColor: "#040f15"`, `image: logo-mark.png` at `imageWidth: 96`) — deliberately just the mark on the flat dark base, not `splash-screen.png`'s full ambient-blob-and-wordmark scene, per the explicit instruction that file is a visual reference only. Also replaced `ios.icon`'s stale `./assets/expo.icon` reference (the untouched default Expo Icon Composer placeholder — still showed a generic blue "A" logo) with the same flattened PNG; a proper Icon Composer `.icon` bundle (iOS 18 tinted/dark variants) would need to be hand-authored in Xcode's Icon Composer, which isn't available in this environment.
- **Photo storage: local Docker volume, not object storage (2026-09-05).** Full reasoning lives in `backend/app/storage.py`'s module docstring — summary: solo/portfolio project on one self-hosted ZimaOS box (Section 6), no multi-region/CDN need, and a named volume (`gymind_uploads`) survives rebuilds exactly like `gymind_pgdata` already does for Postgres. Uploaded files are validated by content-type (`image/jpeg`/`png`/`webp` only) and saved as `<uuid>.<ext>` under `UPLOAD_DIR/<subdir>/`, served statically at `/uploads/...`. If this ever needs to scale past one server, `storage.py`'s two functions (`save_upload`/`delete_upload`) are the only place that would need to change.
- **Nutrition photo attach moved from a generic top-level button to a per-meal-row thumbnail (2026-09-05).** The design's "Attach photo" button wasn't tied to any specific logged meal — its own `photoOpen`/`photoAttached` state in the prototype was a UI demo, not linked to any `nutritionLogs` entry either. The real backend route (`POST /nutrition/{id}/photo`) needs a specific log id, so there's no natural target for a standalone button. Tapping a meal row's thumbnail (showing the photo once attached, a camera icon before that) opens the picker and uploads to that row's log — same interaction, just anchored to a real target instead of a decorative one.
- **Correction (2026-09-05):** an earlier version of this doc claimed the old `Design/exports/` and `Design/Source/` ("5c") folders had been "removed from disk outside of this migration's own work." That was wrong — verified via `git ls-tree HEAD Design/` and `git log origin/main -- Design/exports Design/Source`: both folders are fully intact in git, all 19 files, unarchived, exactly as before. What was actually observed was one local machine's *working-tree* state (the folder was missing from that machine's disk, likely the project's own documented OneDrive-sync risk, not a git operation) — never reflected in any commit. **Standing rule going forward: any claim in this doc about a file being added/removed/moved/committed must be verified against `git status`/`git log` (or a fresh clone) before being written down — not inferred from what a local working directory looks like.** Separately, `Design2/` (the Brand 2.0 handoff itself) *had* genuinely never been committed despite being described as added in the prior update — fixed same day, commit `e011f66`, verified present on `origin/main`.
...
- [x] App icon/splash wired, 2026-09-05 — see Section 10 for the transparency-bug fix and the derived-monochrome-icon technique, both worth knowing about if these assets are ever regenerated.
...
**Iconography:** no icon library used in the design prototype (hand-drawn simple geometric shapes, to avoid a generic "AI slop" look). **Decided 2026-09-05: Lucide** (`lucide-react-native`) — single regular weight, geometric stroke-based line icons (matching the prototype's own `stroke-width`/`stroke-linecap="round"` hand-drawn style far more closely than Phosphor's multi-weight family), built on `react-native-svg` (already a dependency), simple uniform `<Icon size={} color={} />` API, broad ecosystem adoption. Used throughout every screen built in this migration.
...
- ~~**Home:**~~ **done, 2026-09-05.** Greeting + theme toggle, streak pill (client-side computed, see Section 17), Coach teaser card (static link, no coach logic), Nutrition ring card, Workout CTA card (Start/Resume via `WorkoutSessionContext`), Best e1RM stat card (shimmer), body weight stat card.
- ~~**Workout:**~~ **done.** Re-skinned (cut-corner cards, teal/gold, no left-border pattern) with session/state model untouched; rest timer, finish-session menu, repeat workout, past-workout totals, and past-set editing all added (Section 17).
- ~~**Nutrition:**~~ **done**, including real photo upload — calorie ring + 3 macro bars, "+ Log food" bottom sheet, meal photo attach/view per logged meal (`POST /nutrition/{id}/photo`, wired via `expo-image-picker`; see Section 17 for why the affordance moved from the design's generic top-level button to a per-meal-row thumbnail). Empty state built; loading/error states still not designed (open item, unchanged).
- ~~**Progress:**~~ **done**, including real photo upload — consistency ring, body-weight chart, "Progress photos" strip (`POST/GET/DELETE /progress-photos`, same `expo-image-picker` flow, tap a photo to delete), 2-column muscle-group tile grid drilling into an e1RM trend line (client-side merged across the group's exercises, Section 9).
...
- ~~`workout_sets.workout_id`'s `ForeignKey` in `models.py` is missing `ondelete='CASCADE'`~~ — **fixed 2026-09-05**, see Section 10.
...
- ~~Workout screen: rest timer ("REST 2:00") still not built~~ — **done, 2026-09-05.** `openRest(secs=90)` opens a `CenterModal` with a `ProgressRing` countdown (coral, per BRAND_GUIDE.md's "rest timer" alert-color usage), `+15s`/`Skip`. Triggered from marking a set done, skipped when that looks like the last remaining set in the session (`stats.completedSets + 1 < stats.plannedSets`).
- ~~Workout screen: "Finish session" secondary options ("...") menu still not built~~ — **done.** A `MoreHorizontal` icon next to Finish opens a `Sheet` with "Finish workout" / "Discard workout" / "Cancel". Discard deletes the in-progress `UserWorkout` row (if one exists) via the existing `DELETE /workouts/{id}` route, then resets local state — distinct from Finish, which persists it.
- ~~"Repeat this workout" from Past Workouts~~ — **done.** Fetches the past workout's detail, groups its sets by exercise, and repopulates the current exercise list with the same exercises and the same number of *blank* sets (ready to log fresh) — doesn't copy old numbers or start a session.
- ~~Per-workout totals in the Past Workouts list~~ — **done.** `GET /workouts` now returns `total_volume`/`total_sets` per workout (see Section 9/13 for the schema/route change and the `started_at` bug fix that came with it).
- ~~Editing already-logged past sets~~ — **done**, via the new `PUT /workouts/{id}/sets/{set_id}` route (Section 9) and a "past workout detail" bottom sheet with editable weight/reps inputs (saved on blur). Deliberately not blocked by `ended_at`, unlike `POST .../sets` — the whole point is correcting a *finished* session's numbers.
- **Decided (2026-09-05): pre-session set edits auto-start a session, rather than staying ungated or being blocked.** The first time a weight/reps field gets a non-empty value while `hasStarted` is false, `startSession()` fires automatically (same call the "Start session" button uses). Reasoning: blocking edits until Start is pressed adds friction to a common real workflow (prepping your sets before the clock starts); leaving it ungated (the old behavior) risked the user believing typed numbers were saved when they were pure local state, silently lost on nothing more than navigating away. Auto-start removes that ambiguity for free — the moment you log a real number, you're training, matching how most workout-logging apps already behave — and needed no new UI.
