...
- **Register/Login screens are functionally complete, not yet styled:** both call the real backend and are wired into navigation, but still use unstyled placeholder layouts — a visual pass to match `Design/exports/` is separate future work (see Section 11, Section 17). Login (`mobile/src/app/login.tsx`): `POST /auth/login`, saves the returned JWT to `AsyncStorage`, handles both bad-credentials (non-2xx response) and network failures (fetch throwing). Register (`mobile/src/app/register.tsx`): `POST /auth/register`, client-side password validation (8+ characters, at least one uppercase letter) and a password-confirmation check, all run before the request fires.
...
- [x] Build register/login screens wired to `/auth/*` — functionally complete (real backend calls, JWT storage, validation, error handling), **not yet styled to match `Design/exports/`** — that visual pass is separate future work. See Section 10.
...
- [ ] Build the actual onboarding form (goal, experience level, injuries, equipment, dietary restrictions — per Section 15) wired to `/users/profile` — `onboarding.tsx` is currently only a placeholder screen with no real form; this is the next major frontend task
- [ ] Visual design pass for Login, Register, and Onboarding to match the real screens in `Design/exports/` — current versions are functional but visually unstyled placeholders
